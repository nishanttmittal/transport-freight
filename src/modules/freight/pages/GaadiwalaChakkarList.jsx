...
export default function GaadiwalaChakkarList({ transporterId }) {
  ...
  const [open, setOpen] = useState('')   // batchId whose charge breakup is showing
    ...
    <div className="max-w-lg mx-auto px-4 pb-40 space-y-2">
      <div className="pt-3">
        <div className="flex items-baseline justify-between">
          ...
          <div className="text-sm font-bold font-mono text-slate-600">₹{fmtNum(batches.reduce((s, b) => s + b.total, 0))}</div>
        </div>
        <div className="text-[11px] text-slate-400">This period — until your hisab is settled. Tap a chakkar for details.</div>
      </div>
      {batches.length === 0 ? (
        <div className="text-center text-slate-400 text-sm py-6">No chakkars yet — add one above.</div>
      ) : batches.map(b => (
        <Card key={b.batchId} className="p-3" onClick={() => setOpen(o => o === b.batchId ? '' : b.batchId)}>
          ...
          {open === b.batchId
            ? <div className="mt-2"><ChakkarBreakup rows={b.rows} destName={destName} /></div>
